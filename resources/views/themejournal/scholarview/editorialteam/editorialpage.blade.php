...
<body >
    <div class=" p-10 bg-cover bg-top bg-header">
        ...
        <div class="text-center">
            <h1 class="mt-5 ml-3 text-5xl font-bold">{{ $journal->title }}</h1>
        </div>
            ...
            <div class="container box-content p-10 border bg-white shadow-xl w-full ">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <div class="flex items-center">
                            ...
                            <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}/editors">Editorial Team</a></span>
                    </div>
                    </li>
                </ol>
                </nav>
                <hr class="mt-3">
                <h1 class="page_title text-left font-black text-4xl mt-6">Editorial Team</h1>
                <br>
                @foreach ($editor as $item)                    
                <div class="prose prose-neutral max-w-none mt-7 mb-4">
                    <img src="/image/editors/{{ $item->image ?? 'No Data'}}" alt="" style=" height: 200px; width: 200px;">
                    <div class="text-editor">
                    <p class="text-justify mt-2 tracking-wide" style="font-size: 1em; font-family: Roboto,Helvetica Neue,Helvetica,Arial,sans-serif;">
                        Name: {{ $item->name ?? 'No Data' }}
                    </p>
                    <h2 class="page_title text-left text-amber-900" style="font-size: 1.28em; font-family: Lora,serif">Affiliation: {{ $item->affiliation ?? 'No Data'}}</h2>
                    <div class="navigation-page-content" id="navigation-page-content">
                        <p class="text-justify mt-2 tracking-wide" style="font-size: 1em; font-family: Roboto,Helvetica Neue,Helvetica,Arial,sans-serif;">
                            <strong>From: </strong> {{ $item->from ?? 'No Data'}}
                        </p>
                    </div>
                    </div>
                </div>
