    ...
    <div class=" p-10 bg-cover bg-top bg-header">
        ...
        <div class="text-center bg-blue-600">
            ...
            <div class="container box-content p-10 border bg-white shadow-xl w-full ">
                ...
                <hr class="mt-3">
                <h1 class="page_title text-left font-black text-2xl mt-6">Announcement</h1>
                @foreach ($announcement as $item)                    
                <div class="prose prose-neutral max-w-none mt-4">
                    <a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}/announcement/{{ $item->id ?? 'No Data'}}">
                    <h2 class="page_title text-left hover:underline text-blue-900" style="font-size: 1.28em; font-family: Lora,serif">{{ $item->title ?? 'No Data'}}</h2>
                    </a>
                    <div class="navigation-page-content" id="navigation-page-content">
                        <p class="text-justify mt-2 tracking-wide" style="font-size: 0.8em; font-family: Roboto,Helvetica Neue,Helvetica,Arial,sans-serif;">
                            <i class="fa fa-calendar" aria-hidden="true"></i> {{ $item->published ?? 'No Data'}}
                        </p>
                        <p class="text-justify mt-2 tracking-wide" style="font-size: 0.8em; font-family: Roboto,Helvetica Neue,Helvetica,Arial,sans-serif;">
                            {{ substr($item->content ?? 'No Data',0,300) }}...
                        </p>
                    </div>
                </div>
                @endforeach
