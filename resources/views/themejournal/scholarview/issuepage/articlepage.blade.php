...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="{{ URL::asset('css/bootstrap/styles.css') }}" />
        ...
        <div class="h-60 w-full bg-cover pt-10">
            <div class="flex justify-center ">
                <ul class="flex flex-row space-x-16 text-md">
                    <li>
                        <a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}" class="text-white hover-underline-animation  underline-offset-8" aria-current="page">
                            <p>Home</p>
                        </a>
                    </li>
                    <li>
                    ...
                    <nav class="cmp_breadcrumbs flex container mx-auto" role="navigation" aria-label="You are here:">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3">
                            <li class="inline-flex items-center">
                                ...
                                <a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-gray-900 dark:text-gray-400 dark:hover:black-white">Home</a>
                            </li>
                            <li class="current">
                            ...
                            <li class="current">
                                <div class="flex items-center">
                                    ...
                                    @foreach ($article as $item) <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/{{ $account->id }}/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">Article: {{ $item->judul_article ?? 'No Data'}}</a></span> @endforeach
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <hr class="mt-3">
                    <div class="container mt-4" style="text-align:justify; margin-top:10px;">
                        <div class="row">
                            <div class="col-lg-8">
                                ...
                                <article style="text-align: justify">
                                    <header class="mb-4">
                                        <strong>
                                            <h1 class="fw-bolder mb-1 text-4xl text-black-600" style="font-family: Garamond, serif;">{{ $item->judul_article ?? 'No Data' }}</h1>
                                        </strong>
                                        <div class="text-muted fst-italic mb-2 mt-1">Author: {{ $item->author ?? 'No Data'}}</div>
                                    </header>
                                    <figure class="mb-4"><img class="img-fluid rounded" src="/image/issue/article/{{ $item->image ?? 'No Data'}}" style="max-width:1000px; max-height:500px; width:1000px" alt="..." /></figure>
                                    <section class="mb-5">
                                        <h2 class="fw-bolder mb-3 mt-2 text-2xl">Abstract</h2>
                                        <p class="fs-5 mb-4">{{ $item->abstract ?? 'No Data'}}</p>
                                        <h2 class="fw-bolder mb-3 mt-5 text-2xl">Full Article</h2>
                                        <p class="fs-5 mb-4" style="white-space: pre-wrap;">{!! $item->isi ?? 'No Data' !!}</p>
                                    </section>
                                </article>
                                @endforeach
                            </div>
