...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="{{ URL::asset('css/scholartheme/issueselfpage.css') }}" />
    ...
    <div class=" p-10 bg-cover bg-top bg-header">
        <div class="flex justify-around ">
            ...
            <div class=" space-x-3 mt-10">
                <div class="dropdown1">
                <span style="text-transform: capitalize;" class="text-white bg-blue-600 shadow-lg rounded-full text-sm px-8 py-2 mb-2">{{ $account->username ?? ' ' }} ({{ $account->status ?? ' ' }})&nbsp;&nbsp;&nbsp;<i class="fa fa-caret-down"></i></span>
            ...
            <div class="container box-content p-10 border bg-white shadow-xl w-full ">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        ...
                        <a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-900 dark:text-gray-400 dark:hover:text-black">Home</a>
                    </li>
                    <li class="current">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}/issue/archive">Archive</a></span>
                        </div>
                    </li>
                    <li class="current">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            @foreach ($issue as $item)  <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/{{ $account->id }}/journal/{{ $journal->path ?? 'No Data'}}/issue/{{ $item->id ?? 'No Data'}}">{{ $item->volume }}: {{ $item->judul ?? 'No Data'}}</a></span> @endforeach
                        </div>
                    </li>
                </ol>
                </nav>
                ...
                <h1 class="page_title text-left font-black text-2xl mt-6">{{ $item->volume ?? 'No Data'}}: {{ $item->judul ?? 'No Data'}}</h1>@endforeach
                <br>
                @foreach ($issue as $item)
                <div class="prose prose-neutral max-w-none mt-2 mb-4">
                <div class="navigation-page-content" id="navigation-page-content">
                        <p class="text-justify mt-1 mb-3 tracking-wide" style="font-size: 1em; font-family: Roboto,Helvetica Neue,Helvetica,Arial,sans-serif;">
                            <strong>Published: </strong> {{ $item->published ?? 'No Data'}}
                        </p>
                    </div>
                    <img class="image_issue" src="/image/issue/{{ $item->image ?? 'No Data'}}" alt="" style=" height: 216px;">
                </div>
                @endforeach
                <br>
                <strong>
                <h2 class="text-left font-black text-2xl mt-6 article" style="z-index: 1;">Articles</h2>
                </strong>
                <hr style="z-index: 2; margin-top: -13px;">
                <div class="article">
                @foreach($article as $item)
                <div class="blog-card">
                <div class="meta">
                <a href="/{{ $account->id }}/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">
                <img src="/image/issue/article/{{ $item->image ?? 'No Data'}}" class="photo" width="280px" height="100%" alt="">
                </a>
                </div>
                <div class="description">
                <h1><a class="title hover:underline" href="/{{ $account->id }}/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">{{ $item->judul_article ?? 'No Data' }}</a></h1>
                <h2>{{ $item->author ?? 'No Data'}}</h2>
                <p>{{substr($item->abstract ?? 'No Data',0,100)}}...</p>
                <p class="read-more" style="margin-top:10px;">
                    <a href="/{{ $account->id }}/journal/{{$journal->path ?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">Read More</a>
                </p>
                </div>
            </div>
                @endforeach
