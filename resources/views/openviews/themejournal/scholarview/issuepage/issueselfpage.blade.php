            ...
            <div class="container box-content p-10 border bg-white shadow-xl w-full ">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        ...
                        <a href="/journal/{{ $journal->path ?? 'No Data'}}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-900 dark:text-gray-400 dark:hover:text-black">Home</a>
                    </li>
                    <li class="current">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/journal/{{ $journal->path ?? 'No Data'}}/issue/archive">Archive</a></span>
                        </div>
                    </li>
                    <li class="current">
                        <div class="flex items-center">
                            <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                            @foreach ($issue as $item)  <span aria-current="page" class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-black"><a href="/journal/{{ $journal->path ?? 'No Data'}}/issue/{{ $item->id ?? 'No Data'}}">{{ $item->volume }}: {{ $item->judul ?? 'No Data'}}</a></span> @endforeach
                        </div>
                    </li>
                </ol>
                </nav>
                ...
                <div class="blog-card">
                <div class="meta">
                <a href="/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">
                <img src="/image/issue/article/{{ $item->image ?? 'No Data'}}" class="photo" width="280px" height="100%" alt="">
                </a>
                </div>
                <div class="description">
                <h1><a class="title hover:underline" href="/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">{{ $item->judul_article ?? 'No Data' }}</a></h1>
                <h2>{{ $item->author ?? 'No Data'}}</h2>
                <p>{{substr($item->abstract ?? 'No Data',0,100)}}...</p>
                <p class="read-more" style="margin-top:10px;">
                    <a href="/journal/{{$journal->path ?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}">Read More</a>
                </p>
                </div>
            </div>
                @endforeach
