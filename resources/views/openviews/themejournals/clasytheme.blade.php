        ...
        <nav class="navbar navbar-expand-lg navbar-light ">
          ...
          <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav nav-list ml-auto">
              ...
              <a class="menu" href="/journal/{{ $journal->path ?? 'No Data'}}">HOME</a>
              <a class="menu" href="/journal/{{ $journal->path ?? 'No Data' }}/issue/archive">ARCHIVE</a>
              <a class="menu" href="/journal/{{ $journal->path ?? 'No Data'}}/editors">EDITORIAL TEAM</a>
              <a class="menu" href="/journal/{{ $journal->path ?? 'No Data'}}/download">DOWNLOAD</a>
              <span class="menu last-spc color-line"> | </span>
              <div style="width: 200px; height: 40px; float: right;" class="menu dropdown1">
                <li><span style="text-transform: capitalize;"><a href="/home">Login</a>&nbsp;&nbsp;&nbsp;<i class="fa fa-user"></i></span></li>
              </div>
            </div>
          </div>
        </nav>
            ...
            <div class="row">
              <div class="col-sm-12  ">
                <div class="comment-container">
                  <span><a href="/journal/{{ $journal->path ?? 'No Data'}}/editors" style="color: black">Editorial Team</a></span>
                  <hr>
              ...
              <div class="col-md-12  ">
                <div class="comment-container">
                  ...
                  <div class="current_issue_title" style="margin-bottom: 20px;">
                    <strong><a style="color: black" href="/journal/{{ $journal->path ?? 'No Data'}}/issue/{{ $currentissue->id ?? 'No Data'}}">{{ $currentissue->judul ?? 'No Data'}}</a></strong>
                  </div>
                  <div class="heading">
                    <a class="cover" href="/journal/{{ $journal->path ?? 'No Data'}}/issue/{{ $currentissue->id ?? 'No Data'}}">
                    ...
                    <ul class="cmp_article_list articles">
                      <li>
                        <div class="obj_article_summary">
                          <div class="cover">
                            <a href="/journal/{{$journal->path?? 'No Data'}}/article/{{ $item->id ?? 'No Data'}}" class="file">
                      ...
                      @endforeach
                      <a href="/journal/{{ $journal->path ?? 'No Data' }}/issue/archive" class="btn btn-learn" style="margin-left: 570px;">See All <span class="arrow">❯</span></a>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="row ">
              <div class="col-12  side-bar-bg">
                ...
                <div class="categories-link">
                  <a href="/journal/{{ $journal->path ?? 'No Data'}}/announcement/{{ $item->id ?? 'No Data'}}"> <i class="fas fa-angle-right"></i> {{$item->title ?? 'No Data'}} <span class="">({{$item->published ?? 'No Data'}})</span></a>
                </div>
                <hr>
                @endforeach
              </div>
